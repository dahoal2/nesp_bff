...
def convert_temp_unit(da: xr.DataArray) -> xr.DataArray:
    """Ensure temperature is in degC (accepts K or degC)."""
    ...
    if units == "K":
        da = da - 273.15  # CHANGED: 273.16 -> 273.15 (standard)
        da.attrs["units"] = "degC"
    return da


def drop_feb29(ds: xr.Dataset) -> xr.Dataset:
    """
    Drop Feb 29 from any dataset that has it.
    CHANGED: used as a universal first-step so all outputs are no-Feb29.
    """
    mask_feb29 = (ds.time.dt.month == 2) & (ds.time.dt.day == 29)
    return ds.sel(time=~mask_feb29) if bool(mask_feb29.any()) else ds


def cloud_frac_to_oktas(clt_pct: xr.DataArray) -> xr.DataArray:
    """
    Convert cloud cover percentage (0-100) to oktas (0-8).
    Kept for future use if you later bias-adjust cloud cover.
    CHANGED: kept only one clean implementation (replaces duplicate versions).
    """
...
# Core processing
# -------------------------
def process_one(infile: str, outfile: str, ds_nathers_in: xr.Dataset, *, compress: bool = True) -> None:
    # CHANGED: ensure file handle is closed
    with xr.open_dataset(infile) as ds_in:
        # ds = ds_in  # just to keep names short
        keep = ["tas", "huss", "psl", "sfcWind", "rsds", "rsdsdir", "rsdsdif"]
        ds = ds_in[keep].load()
        ds = ds.drop_vars(["lat", "lon"], errors="ignore")

        # CHANGED: always drop Feb 29 at the front for BOTH model and NatHERS
        ds = drop_feb29(ds)
        ds_nathers = drop_feb29(ds_nathers_in)

        # Align early so NatHERS time-dependent fields (slr_alt, wind_dir, clt flags) match model time
        # CHANGED: align once up front for consistency
        # ds, ds_nathers = xr.align(ds, ds_nathers, join="inner")
        ds_nathers = ds_nathers.reindex(time=ds.time)

        # Temperature
        tas = convert_temp_unit(ds["tas"]).rename("tas")
        tas.attrs.update({
        ...
        twbt.attrs["units"] = ds["tas"].attrs.get("units", "K")
        twbt = convert_temp_unit(twbt).rename("twbt")
        twbt.attrs.update({
            "cell_methods": "time: point (interval: 1H)",
            "long_name": "Near-Surface Wet Bulb Temperature",
            "standard_name": "wet_bulb_temperature",
        })

        # Moisture: specific humidity kg/kg -> g/kg
        huss = (ds["huss"] * 1000).rename("huss")
        huss.attrs.update({
            "units": "g/kg",
            "cell_methods": "time: point (interval: 1H)",
        ...
        # Wind: speed from model, direction from NatHERS (as you want)
        wind_speed = ds["sfcWind"].rename("wind_speed")
        ...
        wind_dir = ds_nathers["wind_dir"].rename("wind_dir")
        # wind_dir.attrs["cell_methods"] = "time: point (interval: 1H) area: interpolation (method: bilinear)"
        wind_dir.attrs["comment"] = "No QDC performed. Taken from NatHERS file directly."

        # Radiation: keep diffuse and global from step2 output
        rsdsdif = ds["rsdsdif"].rename("rsdsdif")
        rsdsdif.attrs.update({
            "units": "W m-2",
        ...
        # Convert direct horizontal (rsdsdir) to DNI, but name must remain "rsdsdir" per NatHERS convention.
        # CHANGED: make explicit that input is direct horizontal and output is DNI.
        slr_alt = ds_nathers["slr_alt"]
        proj = np.sin(np.deg2rad(slr_alt))
        min_proj = np.sin(np.deg2rad(5.0))  # ~0.087

        dni_named_rsdsdir = xr.where(
            proj > min_proj,
            ds["rsdsdir"] / proj,
            0.0
        ).rename("rsdsdir")

        dni_named_rsdsdir.attrs.update({
            "units": "W m-2",
            "cell_methods": "time: mean (interval: 1H)",
            "long_name": "Surface Direct Downwelling Shortwave Radiation (DNI)",
            "standard_name": "surface_direct_downwelling_shortwave_flux_in_air",
            "comment": "NOTE: Variable name 'rsdsdir' contains DNI (NatHERS convention).",
        })

        # Cloud: use NatHERS clt but detect representation from value range
        clt_raw = ds_nathers["clt"]
        
        vmax = float(clt_raw.max())
        
        if vmax <= 1.0:
            # fraction (0–1)
            clt_okta = cloud_frac_to_oktas(clt_raw * 100.0).rename("clt")
        elif vmax <= 9.0:
            # already oktas
            clt_okta = clt_raw.clip(0, 8).rename("clt")
        else:
            # percent (0–100)
            clt_okta = cloud_frac_to_oktas(clt_raw).rename("clt")
        
        clt_okta.attrs.update({
            "units": "oktas",
        ...
        # Assemble outputs
        ds_new = xr.merge(
            [tas, twbt, huss, psl, wind_speed, wind_dir, rsds, dni_named_rsdsdir, rsdsdif, clt_okta],
            compat="no_conflicts",
        )
        # Attach NatHERS lat/lon (and keep them in the final file)
        for k in ["lat", "lon"]:
            if k in ds_nathers:
                ds_new = ds_new.assign_coords({k: ds_nathers[k]})
            elif k in ds_nathers.coords:
                ds_new = ds_new.assign_coords({k: ds_nathers.coords[k]})

        # Bring across NatHERS flags/solar angles
        vars_nathers = ds_nathers[[
            "flag-tas", "flag-huss", "flag-psl", "flag-wind_speed",
            "flag-total_cloud_cover", "flag-wind_dir",
            "slr_alt", "slr_azm",
            "flag-rsds", "flag-rsdsdif", "flag-rsdsdir"
        ]]
        
        ds_out = xr.merge([ds_new, vars_nathers], compat="override")

        # Keep NatHERS round_method coord if present
        if "round_method" in ds_nathers.coords:
            ds_out = ds_out.assign_coords(round_method=ds_nathers.round_method)

        # CHANGED: keep output clean
        ds_out.attrs = {}
        ds_out = ds_out.drop_vars(["crs","hour","month"], errors="ignore")

        # (Feb29 already dropped at the top, so no calendar conversion needed)
...
def main():
    ap = argparse.ArgumentParser(
        description="Step3: calc missing NatHERS vars from step2 QDC files (per location)."
    )
    ...
    nathers_file = glob.glob(f"{nathers_dir}{args.loc}_*_NatHERS_UTC_19891231-20151231.nc")
    if not nathers_file:
        raise FileNotFoundError(f"NatHERS file not found for loc={args.loc} in {nathers_dir}")
    # ds_nathers = xr.open_dataset(nathers_file[0]).sel(time=slice("1995", "2014")).load()
    need_nathers = [
        "wind_dir", "clt", "slr_alt", "slr_azm",
        "flag-tas","flag-huss","flag-psl","flag-wind_speed",
        "flag-total_cloud_cover","flag-wind_dir",
        "flag-rsds","flag-rsdsdif","flag-rsdsdir"
    ]
    ds_nathers = xr.open_dataset(nathers_file[0])[need_nathers].sel(time=slice("1995","2014")).load()
...
#     def _has_feb29_time(time) -> bool:
#         return bool(((time.dt.month == 2) & (time.dt.day == 29)).any())
    
#     model_has_feb29 = _has_feb29_time(ds["time"])
...
#     wind_direction = ds_nathers["wind_dir"] #degree_to_compassIndex(wind_dir, sfcWind, calm_threshold=0.026)
...
#     # Convert direct horizontal radiation to DNI
...
#     dni.attrs["long_name"] = "Surface Direct Downwelling Shortwave Radiation (DNI)"
#     dni.attrs["standard_name"] = "surface_direct_downwelling_shortwave_flux_in_air"

#     rsds = ds['rsds'].rename("rsds")#('Global_solar_irradiance')(rsdsdir+rsdsdif)
...
#     # Cloud
#     clt_okta = ds_nathers["clt"]#cloud_frac_to_oktas(ds["clt"])
...
#     ds_out_no_leap = ds_out.sel(time=~mask_feb29) if mask_feb29.any() else ds_out
